...
class PsmEnvMem(SurRoLGoalEnv):
    ...
    def __init__(self,
        ...
        super(PsmEnvMem, self).__init__(render_mode)
    ...
    def reset(self):
        self.obs_buffer = []
        return super(PsmEnvMem, self).reset()

class PsmsEnvMem(PsmsEnv):
    """
    Dual arm environment using PSM1 and PSM2 with memory buffer.
    """
    def __init__(self, render_mode=None):
        self.k = 5  # Number of past frames to include
        self.c = 2  # Skipping interval
        self.obs_buffer = []  # Buffer to store past observations
        super(PsmsEnvMem, self).__init__(render_mode)
    ...
    def _get_obs(self) -> dict:
        ...
        # Handle object-related observations
        if self.has_object:
            pos, _ = get_link_pose(self.obj_id, -1)
            object_pos = np.array(pos)
        ...
        observation = np.concatenate([
            ...
            waypoint_pos2.ravel(), waypoint_rot2.ravel(),
        ])
    ...
    def reset(self):
        self.obs_buffer = []
        return super(PsmsEnvMem, self).reset()
