...
class MisOrientMem(EcmEnvMem):
    ...
    def _env_setup(self):
        super(MisOrientMem, self)._env_setup()
    ...
    @staticmethod
    def _get_MisOrientMemation(tip_world: np.ndarray, tip_world0: np.ndarray) -> np.ndarray:
        """ Get the MisOrientMemation angle """
    ...
    def _get_obs(self) -> dict:
        ...
        mis_orient = self._get_MisOrientMemation(cam_world, self.cam_nof)
    ...
    def get_oracle_action(self, obs) -> np.ndarray:
        ...
        cam_world = get_matrix_from_pose_2d((pos, orn))
        cam_world[:3, 3] /= self.SCALING
        droll = self._get_MisOrientMemation(cam_world, self.cam_nof) / np.deg2rad(3)
        action = droll.clip(-1, 1) * 0.3
        return action


if __name__ == "__main__":
    env = MisOrientMem(render_mode='human')  # create one process and corresponding env
